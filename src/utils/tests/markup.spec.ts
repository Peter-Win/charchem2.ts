import { MarkupChunk, MarkupChunkType, markupFlat, ParamsMarkupFlat, parseMarkup, scanMarkupEnd } from "../markup";
...
describe("markupFlat", () => {
    const mkTag = (type: MarkupChunkType): string => {
        return type || "span";
    }
    const toHtml = (src: string): string => {
        const m = parseMarkup(src);
        let result = "";
        let prevColor: string | undefined = undefined;
    ...
    it("sub- and super-script", () => {
        expect(toHtml("A_{11}^2 + B_{12}^3")).toBe("A<sub>11</sub><sup>2</sup> + B<sub>12</sub><sup>3</sup>");
    })
    it("color", () => {
        expect(toHtml("A, \\color{red}B.")).toBe(`A, <span style="color: red">B.</span>`);
        expect(toHtml("A, {\\color{green}Green}, C")).toBe(`A, <span style="color: green">Green</span>, C`)
    })
})