...
const drawMarkup = ({chunk, imgProps, font, style, scale}: ParamsDrawMarkup): ResultTextWithMarkup => {
    ...
    chunk.chunks.forEach(subChunk => {
        ...
        if (type === "sup" || type === "sub") {
            ...
            const newFont: LocalFont = font.createScaled ? font.createScaled(scale) : font;
            const ff = newFont.getFontFace();
            const height = ff.ascent - ff.descent;
            const rs = drawMarkup({chunk: subChunk, font: newFont, style, scale, imgProps});
            const rsFig = rs.fig;
            if (isSup) {
        ...
        const res = drawMarkup({chunk: subChunk, imgProps, font, style: newStyle, scale});
...
export const drawTextWithMarkup = (text: string, imgProps: ChemImgProps, {font, style}: TextProps): ResultTextWithMarkup => {
