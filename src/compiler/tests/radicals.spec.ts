...
describe("radicals", () => {
  it("Me", () => {
    ...
    expect(makeTextFormula(expr)).toBe("Me");
  ...
  it("Et2O", () => {
    ...
    expect(expr.getMessage()).toBe("");
    expect(makeTextFormula(expr)).toBe("Et2O");
    const { dict } = PeriodicTable;
  ...
  it("WithBrackets", () => {
    ...
    expect(makeTextFormula(expr)).toBe("i-BuOH");
    const { dict } = PeriodicTable;
  ...
  it("AcetylCoA", () => {
    ...
    expect(expr.getMessage()).toBe("");
    expect(makeTextFormula(makeBrutto(expr))).toBe("C23H38N7O17P3S");
    expect(makeTextFormula(makeBrutto(compile("{CoA}SH")))).toBe(
      "C21H36N7O16P3S"
    );
  });
});
