...
describe("draftGraphFromAutoNode", () => {
  it("linear with different multipicity", () => {
    ...
    expect(g0.vertices.length).toBe(4);
    expect(g0.edges.length).toBe(3);
    expect(g0.vertices[0]!.valence).toBe(4);
    expect(g0.vertices[0]!.reserved).toBe(1);
    expect(g0.vertices[1]!.valence).toBe(1);
    expect(g0.vertices[1]!.reserved).toBe(0);
    // C
    const g1 = draftGraphFromAutoNode(nodes[1]!);
    expect(g1.getElemList().toString()).toBe("C");
    expect(g1.edges.length).toBe(0);
    expect(g1.vertices.length).toBe(1);
    expect(g1.vertices[0]!.valence).toBe(4);
    expect(g1.vertices[0]!.reserved).toBe(4);
    // CH
    const g3 = draftGraphFromAutoNode(nodes[3]!);
    expect(g3.getElemList().toString()).toBe("CH");
    expect(g3.edges.length).toBe(1);
    expect(g3.vertices.length).toBe(2);
    expect(g3.vertices[0]!.valence).toBe(4);
    expect(g3.vertices[0]!.reserved).toBe(3);
    expect(g3.vertices[1]!.valence).toBe(1);
    expect(g3.vertices[1]!.reserved).toBe(0);
    // CH2
    const g4 = draftGraphFromAutoNode(nodes[4]!);
    expect(g4.getElemList().toString()).toBe("CH2");
    expect(g4.edges.length).toBe(2);
    expect(g4.vertices.length).toBe(3);
    expect(g4.vertices[0]!.valence).toBe(4);
    expect(g4.vertices[0]!.reserved).toBe(2);
    expect(g4.vertices[1]!.valence).toBe(1);
    expect(g4.vertices[1]!.reserved).toBe(0);
  });
  it("branches", () => {
    //  0 \ 1/ 3
    //     ||
    ...
    expect(g2.vertices[0]?.reserved).toBe(2);
  });
});
